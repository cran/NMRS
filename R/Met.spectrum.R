...
function(xCoords)
{
  ...
  main<-paste("Spectrum ",espectro$a)
  indexLabeled<-c()
  labeledPoints <- list()
 ...
 tt <- tktoplevel(background="white")
  ...
  {
  params <- par(bg="white")
  plot(xCoords[,1],xCoords[,espectro$a],type=tipo,xlab=xlab,ylab=ylab,xlim=xlim,main=main,
...
tkdestroy(dlg)
main<<-paste("Spectrum ",as.numeric(tclvalue(SliderValue1)))
tkrreplot(img,plotFunction())
