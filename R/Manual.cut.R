...
function(coordenadas)
{
    ...
    cut <- function() {
        ...
        slider1 <- tkscale(dlg, from = round(dat[1, 1], 3), to = round(dat[dim(dat)[1], 
            1], 3), showvalue = F, variable = SliderValue1, resolution = 0.001, 
            orient = "horizontal", length = 300, bigincrement = 0.001)
        slider2 <- tkscale(dlg, from = round(dat[1, 1], 3), to = round(dat[dim(dat)[1], 
            1], 3), showvalue = F, variable = SliderValue2, resolution = 0.001, 
            orient = "horizontal", length = 300)
        slider3 <- tkscale(dlg, from = round(dat[1, 1], 3), to = round(dat[dim(dat)[1], 
            1], 3), showvalue = F, variable = SliderValue3, resolution = 0.001, 
            orient = "horizontal", length = 300, bigincrement = 0.001)
        slider4 <- tkscale(dlg, from = round(dat[1, 1], 3), to = round(dat[dim(dat)[1], 
            1], 3), showvalue = F, variable = SliderValue4, resolution = 0.001, 
            orient = "horizontal", length = 300)
        cb <- tkcheckbutton(dlg)
        cbValue <- tclVar("0")
        tkconfigure(cb, variable = cbValue)
        tkgrid(tklabel(dlg, text = "       "))
        tkgrid(tklabel(dlg, text = "Select the spectral region to analyze"), 
            sticky = "w")
        tkgrid(tklabel(dlg, text = "       "))
        tkgrid(tklabel(dlg, text = "                     From : "), 
            SliderValueLabel1, slider1)
        tkgrid(tklabel(dlg, text = "                     To : "), 
            SliderValueLabel2, slider2)
        tkgrid(tklabel(dlg, text = "       "))
        tkgrid(tklabel(dlg, text = "Delete any spectral region "), 
            cb)
        tkgrid(tklabel(dlg, text = "       "))
        tkgrid(tklabel(dlg, text = "                     From : "), 
            SliderValueLabel3, slider3)
        tkgrid(tklabel(dlg, text = "                     To : "), 
            SliderValueLabel4, slider4)
        onOK <- function() {
ReturnVal <<- 1
            ...
            if (as.numeric(tclvalue(SliderValue1)) < as.numeric(tclvalue(SliderValue2))) {
                xlim <<- c(as.numeric(tclvalue(SliderValue2)), 
                  as.numeric(tclvalue(SliderValue1)))
            }
...
if (is.na(pos$C)==TRUE|is.na(pos$D)==TRUE)
{
   tkmessageBox(message="The selected area has been removed previously",icon="error",type="ok")
stop("The selected area has been removed previously") 
        }
        dat <- dat[-c(1:pos$A, pos$C:pos$D, pos$B:dim(dat)[1]),]
    }
